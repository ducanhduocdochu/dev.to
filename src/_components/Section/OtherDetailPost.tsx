...
const OtherDetailPost: FC<OtherDetailPostProps> = ({ data_user, id, session }) => {
  ...
  const tabBoxs: Tab[] = [
    {
      ...
      sub: "",
      isLoading: isLoading_postByUser,
      posts: data_postByUser ? data_postByUser.posts : [],
    },
    {
      id: 2,
  ...
  return (
    <div className='w-[375.594px] h-[100000px]'>
      ...
      <Button type="secondary" className="" classNameProp="!p-0 h-[50px] absolute hover:!bg-transparent hover:no-underline hover:!text-button4 flex items-end w-full top-[-18px] left-[16px]">
            <img
              src={data_user.image || ""}
        ...
        {session && session.user && session.user.id === id ? (
          <Button type="primary" classNameProp="justify-center bg-[#3b49df] hover:no-underline hover:!bg-button4 text-white" className="mt-[34px]">
            Edit profile
          </Button>
        ) : (
          <Button type="primary" classNameProp="justify-center bg-[#3b49df] hover:no-underline hover:!bg-button4 text-white" className="mt-[34px]">
        ...
        <p className="text-text mt-4 mb-4">I'm backend developer</p>
        <div>
