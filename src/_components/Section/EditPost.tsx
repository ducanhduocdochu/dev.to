...
const EditPost: FC<{
  ...
  const mutationp = api.post.updatePost.useMutation();
  const mutationD = api.post.deletePost.useMutation();
  ...
  useEffect(() => {
    if (data_post) {
      setTitle(data_post.post.title);
      setContent(data_post.post.content);

      const newTags = data_post.post.tags
        .map((item) => {
          const tag = data_tags?.find((it) => it.id == item.tagId);
          if (tag) {
            return { id: item.tagId, name: tag.name, color: tag.color };
          }
          return null;
        })
  ...
  const handleSubmit = async () => {
    if (title == "") {
      ...
      setIsError("");
      router.push(`/${user_id}/${post_id}`).catch((error) => {
      ...
      <div>
        <div className="flex h-header-h w-[956.391px] justify-center">
          ...
          <div className="flex w-header-w justify-between">
            <div className="flex items-center">
              ...
              <h1 className="ml-[15px] font-medium">Edit Post</h1>
            </div>
            <div className="flex items-center">
              <Button
                type="secondary"
        ...
        {isEdit ? (
          ...
          <>
            ...
            {isOpenOption && (
                <Box classNameProp="absolute bg-white top-[-170%] right-[530px] z-99 border border-[#aaaaaa] shadow-xl">
                  <h1 className="mb-4 text-text font-bold">Post Options</h1>
                  <Button
                    type="secondary"
                    className=""
              ...
              <Button
                type="primary"
                className="ml-[80px]"
                classNameProp="w-max bg-button p-2 text-white w-[84px] hover:no-underline mt-[24px] hover:bg-[#2f3ab2]"
              ...
              <Button
                type="secondary"
                className="ml-[20px]"
                classNameProp="w-max bg-button p-2 text-white w-[84px] bg-white hover:no-underline mt-[24px] hover:bg-[#2f3ab2]"
